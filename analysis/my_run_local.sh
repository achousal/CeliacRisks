...
if [[ ${POSTPROCESS_ONLY} -eq 1 ]]; then
  ...
  for MODEL in "${MODEL_ARRAY[@]}"; do
    ...
    if [[ ${DRY_RUN} -eq 1 ]]; then
      ...
      # Use .live log to indicate running job
      LOG_BASE="${LOGS_DIR}/${JOB_NAME}_$(date +%Y%m%d_%H%M%S)"
      LOG_FILE="${LOG_BASE}.log"
      LOG_LIVE="${LOG_BASE}.live"
      ...
      if ced train \
        ...
        --outdir "${RESULTS_DIR}" \
        2>&1 | tee "${LOG_LIVE}"; then

        # Success: rename .live to .log
        mv "${LOG_LIVE}" "${LOG_FILE}"
        END_TIME=$(date +%s)
        ELAPSED=$((END_TIME - START_TIME))
        log "  ✓ ${MODEL}: Complete (${ELAPSED}s)"
        COMPLETED_MODELS+=("${MODEL}")
      else
        # Failure: rename .live to .log anyway
        [[ -f "${LOG_LIVE}" ]] && mv "${LOG_LIVE}" "${LOG_FILE}"
