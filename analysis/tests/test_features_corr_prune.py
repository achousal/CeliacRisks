...
class TestPruneAndRefillPanel:
    ...
    def test_refill_metadata(self, basic_data):
        ...
        ranked = ['PROT_A', 'PROT_B', 'PROT_C', 'PROT_D']
        selection_freq = {p: 0.5 for p in ranked}
        ...
        refilled = [p for p in final_panel if df_map[df_map['protein'] == p]['component_size'].iloc[0] == 1]
        ...
        for protein in refilled:
            ...
            assert row['kept'] == True  # Use == for numpy bool comparison
            assert row['representative_flag'] == True
