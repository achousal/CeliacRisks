...
def test_no_duplicate_build_models_function():
    ...
    assert 'build_models' not in cli_functions, \
        "CLI should import build_models, not define it"

    # Should NOT have get_param_distributions (imported from models.hyperparams)
    assert 'get_param_distributions' not in cli_functions, \
    ...
    assert 'oof_predictions_with_nested_cv' not in cli_functions, \
...
def test_no_duplicate_calibration_function():
    ...
    assert 'maybe_calibrate_estimator' not in cli_functions, \
        "CLI should import maybe_calibrate_estimator, not define it"
    assert 'calibration_intercept_slope' not in cli_functions, \
...
def test_no_duplicate_prevalence_function():
    ...
    assert 'PrevalenceAdjustedModel' not in cli_classes, \
...
def test_registry_module_identity():
    ...
    assert build_models is orig_build, \
        "build_models should be imported, not duplicated"
    assert parse_class_weight_options is orig_parse, \
...
def test_hyperparams_module_identity():
    ...
    # Should be the exact same function object (not a copy)
    assert get_param_distributions is orig_params, \
...
def test_training_module_identity():
    ...
    # Should be the exact same function object (not a copy)
    assert oof_predictions_with_nested_cv is orig_oof, \
...
def test_calibration_module_identity():
    ...
    # Should be the exact same function object (not a copy)
    assert maybe_calibrate_estimator is orig_calib, \
        "maybe_calibrate_estimator should be imported, not duplicated"
    assert calibration_intercept_slope is orig_slope, \
...
def test_prevalence_module_identity():
    ...
    assert PrevalenceAdjustedModel is orig_prev, \
...
def test_models_modules_available():
    ...
    for func_name in registry_funcs + hyperparams_funcs + training_funcs + calibration_funcs + prevalence_funcs:
        assert hasattr(train, func_name), \
            f"CLI train should import {func_name} from models module"
