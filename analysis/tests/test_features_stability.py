...
- build_frequency_panel: both rules (topN, freq_ge_tau)
...
class TestBuildFrequencyPanel:
    """Tests for build_frequency_panel."""

    def test_topN_rule(self):
        """TopN rule keeps N highest-frequency proteins."""
        freqs = {'A': 0.90, 'B': 0.80, 'C': 0.70, 'D': 0.60}

        panel = build_frequency_panel(freqs, rule='topN', top_n=2)

        assert len(panel) == 2
        assert panel['protein'].tolist() == ['A', 'B']
        assert panel['selection_freq'].tolist() == [0.90, 0.80]
        assert panel['rank'].tolist() == [1, 2]

    def test_freq_ge_tau_rule(self):
        """freq_ge_tau keeps proteins with freq >= threshold."""
        freqs = {'A': 0.90, 'B': 0.80, 'C': 0.70, 'D': 0.60}

        panel = build_frequency_panel(freqs, rule='freq_ge_tau', freq_threshold=0.75)

        assert len(panel) == 2
        assert panel['protein'].tolist() == ['A', 'B']
        assert panel['selection_freq'].tolist() == [0.90, 0.80]

    def test_sorting_by_freq_then_name(self):
        """Proteins sorted by frequency DESC, then name ASC."""
        freqs = {'PROT_C': 0.90, 'PROT_A': 0.90, 'PROT_B': 0.80}

        panel = build_frequency_panel(freqs, rule='topN', top_n=10)

        # Same frequency: alphabetical order
        assert panel['protein'].tolist() == ['PROT_A', 'PROT_C', 'PROT_B']
        assert panel['selection_freq'].tolist() == [0.90, 0.90, 0.80]
        assert panel['rank'].tolist() == [1, 2, 3]

    def test_empty_frequencies(self):
        """Empty frequencies returns empty DataFrame."""
        panel = build_frequency_panel({}, rule='topN', top_n=10)

        assert panel.empty
        assert list(panel.columns) == ['protein', 'selection_freq', 'rank']

    def test_unknown_rule(self):
        """Unknown rule raises ValueError."""
        freqs = {'A': 0.90}

        with pytest.raises(ValueError, match="Unknown panel rule"):
            build_frequency_panel(freqs, rule='unknown_rule')

    def test_top_n_larger_than_pool(self):
        """topN larger than pool size returns all proteins."""
        freqs = {'A': 0.90, 'B': 0.80}

        panel = build_frequency_panel(freqs, rule='topN', top_n=100)

        assert len(panel) == 2

    def test_freq_threshold_no_matches(self):
        """freq_ge_tau with no matches returns empty panel."""
        freqs = {'A': 0.50, 'B': 0.40}
...
class TestExtractStablePanel:
    ...
    def test_stable_panel_basic(self):
        ...
        log = pd.DataFrame({
            ...
            'fold': [0, 1, 0, 1, 0, 1],
            'selected_proteins_split': [
                '["A", "B"]', '["A", "C"]',  # repeat 0: {A, B, C}
                '["A", "B"]', '["A", "D"]',  # repeat 1: {A, B, D}
                '["A", "B"]', '["B", "C"]',  # repeat 2: {A, B, C}
            ],
        })
...
class TestIntegrationScenarios:
    ...
    def test_full_workflow(self):
        ...
        # Step 2: Build topN panel
        panel = build_frequency_panel(split_freqs, rule='topN', top_n=3)
        assert len(panel) == 3
        assert 'A' in panel['protein'].values

        # Step 3: Extract stable panel (repeat-level)
